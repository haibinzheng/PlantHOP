...
if (dir.exists(run_dir) || dir.exists(partial_dir)) {
  stop("Refusing to overwrite existing v2 run or partial directory")
}
...
for (contrast_index in seq_len(nrow(freeze))) {
  ...
  complete_sample_rows <- accumulated_sample_rows[
    accumulated_sample_rows$contrast_id == contrast_id &
...
derived_path <- file.path(derived_dir, "GSE232863_top128_orthogroup_v1.rds")
saveRDS(
  list(
    matrix = encoded,
...
audit <- list(
  ...
  status = "independent_program_validation_v2_complete",
...
rm(obj, rna, counts)
invisible(gc())
if (!file.rename(partial_dir, run_dir)) stop("Failed to atomically finalize v2 run directory")

file.copy(file.path(run_dir, "validation_summary.tsv"), file.path(meta_dir, "independent_program_validation_summary_v2.tsv"), overwrite = FALSE)
file.copy(file.path(run_dir, "method_comparison_pooled.tsv"), file.path(meta_dir, "independent_program_method_comparison_pooled_v2.tsv"), overwrite = FALSE)
file.copy(file.path(run_dir, "method_comparison_by_sample.tsv"), file.path(meta_dir, "independent_program_method_comparison_by_sample_v2.tsv"), overwrite = FALSE)
file.copy(file.path(run_dir, "audit.json"), file.path(report_dir, "independent_program_validation_v2_audit.json"), overwrite = FALSE)
